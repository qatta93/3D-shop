...
export const ProductCard = ({item}) => {
  ...
  return (
    <div className="w-full mb-12 border-1 bg-white border-indigo-600 shadow-xl md:w-[600px] xl:w-[500px] xl:mx-12 md:rounded-xl cursor-pointer">
      {showModel === 1 &&      
      <section className='relative w-full h-full' onClick={() => setShowDetails(!showDetails)}>
      ...
      {showModel === 3 &&
        <section className="relative w-full h-full">
          <iframe className="relative w-full h-full" title="Comfy Chair" frameBorder="0" allowFullScreen allow="autoplay; fullscreen; xr-spatial-tracking" xr-spatial-tracking execution-while-out-of-viewport execution-while-not-rendered web-share src={item.embed} />
        </section>
      }
